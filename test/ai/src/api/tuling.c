...
int tl_reboot(char *input, char **result, size_t *outlen)
{
	...
	size_t postsize = 0;
	//post的数据长度
	int postDataLen = 18 + strlen(api_set.tuling_key) + strlen(api_set.tuling_uid)
			+ strlen(input);
	char *postdata = (char *) malloc(postDataLen + 1);
	snprintf(postdata, postDataLen, "key=%s&info=%s&userid=%s", api_set.tuling_key,
			input, api_set.tuling_uid);
	//通过HTTP POST的方式提交数据
